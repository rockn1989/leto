...
/*_____ Delete unused css _____*/

gulp.task("purgecss", () => {
	return gulp
		.src("app/libs/uikit-3/css/uikit.min.css")
		.pipe(
			purgecss({
				content: ["build/app/*.html"],
				safelist: [/@m$/, /@s$/, /@lg$/],
			})
		)
		.pipe(gulp.dest("build/app/libs/uikit-3/css/"));
});
...
function browserSync() {
	...
	gulp
		.watch("app/img/icon-svg/sprite/sprite-svg.svg")
...
function Watch() {
	gulp.watch("app/templates/**/*.twig", gulp.series("twig"));
}

/*______ Twig ______*/

function Twig() {
	return gulp.src("app/templates/*.twig").pipe(twig()).pipe(gulp.dest("app/"));
}
...
function SvgstoreBuild() {
	return gulp
		.src("build/app/img/icon-svg/*.svg")
		.pipe(
			svgstore({
				inlineSvg: true,
			})
...
function CleanBuild() {
	return gulp
		.src("build/app/img/icon-svg/sprite/sprite-svg.svg", { read: false })
